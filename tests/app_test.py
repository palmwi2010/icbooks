...
def test_get_books_with_mock_file():
    ...
    mock_json_data = '{"books": [{"title": "Book Title", "isbn": "123456789", "authors": "Anonymous"}]}'
    ...
    assert books[0]["url"] == "https://covers.openlibrary.org/b/isbn/123456789-M.jpg"
...
def test_books_have_correct_urls():
    ...
    expected_url_first = "https://covers.openlibrary.org/b/isbn/" + books[0]["isbn"] + "-M.jpg"
    assert books[0]["url"] == expected_url_first
    expected_url_last = "https://covers.openlibrary.org/b/isbn/" + books[=1]["isbn"] + "-M.jpg"
